...
public:
    ...
    // 因为begin() 和 end()是内联函数，因此在哪里被调用，才在哪里初始化，在ch16_main.cpp的prog3中被调用
    // 此时文件中已经包含BlobPtr<T>的完整定义，因此可以正确编译，内联函数的好处啊
    BlobPtr<T> begin() const {  return BlobPtr<T>(*this, 0); }
    BlobPtr<T> end() const   {  return BlobPtr<T>(*this, size()); }

private:
    void check(std::size_t index, const std::string &msg) const;

    std::shared_ptr<std::vector<T>> _data;  // 指向vector数组的智能指针
};

template <typename T>
template <typename Iterator>
Blob<T>::Blob(Iterator bgit, Iterator edit):
    _data(std::make_shared<std::vector<T>>(bgit, edit)) {}
