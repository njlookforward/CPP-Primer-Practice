...
TextQuery::TextQuery(ifstream &inputfile) : _ptext(new vector<string>(), DebugDelete())
{
    // 需要按行存储
    string line;
    while (getline(inputfile, line))
    {
        ...
        while (iss >> word)
        {
            ...
            if(!occurSet)
                occurSet.reset(new set<line_no>(), DebugDelete());   // shared_ptr的reset只能接受内置指针类型，不能传递shared_ptr
...
{
    static shared_ptr<set<line_no>> nullset(new set<line_no>(), DebugDelete());
