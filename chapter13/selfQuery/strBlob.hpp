...
public:
    // strBlob() : _data(std::make_shared<std::vector<String>>()) {}
    strBlob(): _data(std::make_shared<strVec>()) {}
    explicit strBlob(std::initializer_list<String> il) :    // initializer_list is also class template
    ...
    String &front();
    String &back();
    const String &front() const;   // can use const qualifier to overload
...
private:
    void check(size_t, String) const;

    // std::shared_ptr<std::vector<String>> _data;    // 智能指针指向可被多个strBlob对象共享的vector<string>
...
public:
    ...
    strBlobPtr &operator--();
    strBlobPtr operator--(int);
    const String &deref(size_t dis) const;

private:
    std::shared_ptr<strVec> check(size_t pos, String) const;
