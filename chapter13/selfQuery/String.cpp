...
void String::reallocate()
{
    ...
    // 为什么strVec需要使用make_move_iterator()，而String不需要使用，因为strVec中是string数组，需要移动string的资源
    // 而在String中，是char数组，只需要单纯复制即可，没有资源需要移动，因此不需要移动操作
    auto newFirFree = uninitialized_copy(_elements, _first_free, newbg);
    // 应该用移动构造函数，可以提高效率，存在问题，现在脑子宕机了，需要好好考虑
    // auto newFirFree = uninitialized_copy(make_move_iterator(_elements),
                            //    make_move_iterator(_first_free), newbg);
    free();
    _elements = newbg;
