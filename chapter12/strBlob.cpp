...
{
    shared_ptr<vector<string>> sp = check(_pos,"dereference fail");
    return (*sp)[_pos];
}
...
shared_ptr<vector<string>> strBlobPtr::check(size_t pos, const string &msg) const
{
    // 经过多重检查
    shared_ptr<vector<string>> sp = _pd.lock();
    if(!sp)
...
std::shared_ptr<std::vector<std::string>> ConstStrBlobPtr::check(const std::string &msg) const
{
    auto ret = _pcd.lock();
    if(!ret)
