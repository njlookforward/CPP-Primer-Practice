...
public:
    ...
    strBlob(std::vector<std::string> *pvs): _data(pvs) {}   // 为了满足TextQuery的构造函数的形式，再添加一个新的
    strBlob(const strBlob &rhs) : _data(rhs._data) {}
    ~strBlob() {}
...
private:
    std::shared_ptr<std::vector<std::string>> check(size_t pos, const std::string &) const;

    std::weak_ptr<std::vector<std::string>> _pd;    // strBlob._data的伴随指针类
    size_t _pos;    // 指针的位置索引
};
...
private:
    ...
    std::weak_ptr<std::vector<std::string>> _pcd;
    size_t _pos;
};
